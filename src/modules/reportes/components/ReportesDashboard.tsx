...
import { exportarInventarioPDF, exportarPatrimonioPDF, exportarPDFVentas } from '../utils/pdfGenerators'
...
export default function ReportesDashboard() {
  ...
  const generarVentas = async () => {
    setLoading(true)
    try {
      const data = await ReporteService.obtenerReporteVentas(fi, ff)
      setReporte(data)
    } catch (e: any) {
  ...
  const exportarInventario = () => {
    exportarInventarioPDF(invReporte)
  }

  const exportarPatrimonio = async () => {
    setPatriLoading(true)
    try {
      const d = await ReporteService.obtenerReportePatrimonio()
      exportarPatrimonioPDF(d)
    } catch (e: any) {
      toast.error(e.message || 'Error al generar reporte')
    }
  ...
  const loadInventario = async () => {
    setInvLoading(true)
    try {
      const data = await ReporteService.obtenerReporteInventario()
      setInvReporte(data)
    } catch (e: any) {
      toast.error(e.message || 'Error al generar valoración')
    }
    setInvLoading(false)
  }
  ...
  const exportarVentas = () => {
    ...
    setPdfLoading(true)
    exportarPDFVentas(reporte, fi, ff)
          ...
          {invReporte && (
            <>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button className="btn-ghost" onClick={exportarInventario} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  Exportar PDF
                </button>
              </div>
              {/* KPIs */}
      ...
      {tabReporte === 'ventas' && (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {reporte && (
              <button className="btn-ghost" onClick={exportarVentas} disabled={pdfLoading}>
          ...
          <div className="card" style={{ padding: 16 }}>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              ...
              <button className="btn-primary" onClick={generarVentas} disabled={loading}>{loading ? 'Generando...' : 'Generar reporte'}</button>
          ...
          {!reporte && !loading && (
            <div style={{ textAlign: 'center', padding: 80, color: '#94a3b8' }}>
              ...
              <button className="btn-primary" onClick={generarVentas}>Generar reporte</button>
            </div>
          )}
        </>
      )}
