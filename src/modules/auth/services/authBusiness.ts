...
export async function authorizeUser(credentials: any): Promise<AuthSessionUser | null> {
  if (!credentials?.usuario || !credentials?.password) return null

  const user = await authRepository.findUsuarioByNombre(credentials.usuario as string)
  if (!user) return null
...
export async function refreshTokenClaims(token: any) {
  if (token && token.id) {
    try {
