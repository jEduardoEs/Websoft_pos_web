...
<div style="text-align:right;font-size:10px;color:#64748b">Orden: <b style="color:#2563eb;font-size:14px">${orden.numero || '—'}</b><br>Fecha: ${safeDate(orden.fecha)}</div>
</div>
...
<div>
  ...
  <div class="row"><span class="lbl">Accesorios:</span><span class="val">${orden.accesorios || 'Sin accesorios'}</span></div>
  <div class="row"><span class="lbl">Fecha promesa:</span><span class="val">${safeDate(orden.fechaPromesa)}</span></div>
</div>
</div>
<div class="falla"><strong>Descripción de la falla:</strong><br>${orden.descripcionFalla || 'Sin detalles de falla'}</div>
