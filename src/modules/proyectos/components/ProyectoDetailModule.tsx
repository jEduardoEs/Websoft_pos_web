                      ...
                      <label style={{ display: 'block', marginTop: 8, fontSize: 11, color: '#1581E3', fontWeight: 600, cursor: uploading ? 'wait' : 'pointer' }}>
                        ...
                        <input type="file" accept="image/*" style={{ display: 'none' }} disabled={uploading}
                          onChange={async e => {
                            ...
                            setUploading(false)
                            if (d.url) {
                              // We use fetch directly here to avoid circular dependencies and keep it simple, since it's a specific action
                              await fetch(`/api/proyectos/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ accion: 'agregar_imagen', mantId: m.id, imagenes: [d.url] }) })
                              // No `load()` from hook available easily here, but we can trigger a re-render or let it be. Wait, `load()` is returned! 
                              // I didn't export `load()` from the hook. Let me just use window.location.reload() or I'll add `load()` to hook.
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input className="input" placeholder="Ej: GAR-000001" style={{ flex: 1, maxWidth: 200 }}
                onKeyDown={async e => {
                  ...
                  if ((await res.json()).ok) { toast.success('Garantía vinculada'); window.location.reload() }
