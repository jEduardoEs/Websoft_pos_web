...
export function PosGrid({
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid #e2e8f0', background: '#fff' }}>
      {/* HEADER PESTAÑAS */}
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #e2e8f0', display: 'flex', gap: 6, background: '#f8fafc', alignItems: 'center', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
        ...
        {tab === 'inventario' && (
          <>
            ...
            <div style={{ padding: 10, display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: 8, alignContent: 'start' }}>
              {productos.map(p => (
                <div
                  key={p.id}
          ...
          <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="number" step="0.01" min="0" className="input" value={libreForm.precio} onChange={e => setLibreForm(p => ({ ...p, precio: e.target.value }))} placeholder="0.00" />
              </div>
              <div style={{ width: 80 }}>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: 4 }}>Cant.</label>
                <input type="number" min="1" step="1" className="input" value={libreForm.cantidad} onChange={e => setLibreForm(p => ({ ...p, cantidad: e.target.value }))} />
              </div>
            </div>
