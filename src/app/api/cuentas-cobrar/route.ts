...
export async function GET(req: NextRequest) {
  try {
    ...
    const resultado = await CuentasBackendService.getCuentasCobrar(estado);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const cuenta = await CuentasBackendService.crearCuentaCobrar(body, session.user);
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const cuenta = await CuentasBackendService.cobrarCuentaCobrar(Number(id), montoPago, notas, session.user);
    return NextResponse.json({ ok: true, cuenta });
  } catch (e: any) {
