...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Auto-save client as prospect if has phone/name
    if (clienteNombre && clienteNombre !== 'Consumidor Final') {
      try {
        const existente = await prisma.cliente.findFirst({
          where: {
            OR: [
              clienteTelefono ? { telefono: clienteTelefono } : { nombre: clienteNombre },
              clienteNit && clienteNit !== 'CF' ? { nit: clienteNit } : { nombre: clienteNombre },
            ]
          }
        })
        if (!existente) {
          await prisma.cliente.create({
            data: {
              ...
              notas: `Prospecto desde cotización ${cotizacion.numero}`,
            }
          })
        }
      } catch { /* non-critical */ }
