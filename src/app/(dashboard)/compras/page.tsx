  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a1f36' }}>Compras</h1>
          <p style={{ fontSize: 13, color: '#4a5568', marginTop: 3 }}>Órdenes de compra a proveedores</p>
        </div>
      ...
      <div className="card">
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {compras.length === 0 ? <tr><td colSpan={5} style={{ textAlign: 'center', padding: 40, color: '#9ca3af' }}>Sin compras</td></tr>
              : compras.map(c => (
                <tr key={c.id}>
                  <td style={{ padding: '10px 13px', fontWeight: 700, fontSize: 13, borderBottom: '1px solid #f4f7fb', color: '#2B7FD4' }}>{c.numero}</td>
                  <td style={{ padding: '10px 13px', fontSize: 12, borderBottom: '1px solid #f4f7fb', color: '#4a5568', whiteSpace: 'nowrap' }}>{fmtDateTime(c.fecha)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f4f7fb' }}>{c.proveedorNombre || '—'}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, fontWeight: 700, borderBottom: '1px solid #f4f7fb' }}>{fmt(c.total)}</td>
                  <td style={{ padding: '10px 13px', borderBottom: '1px solid #f4f7fb' }}><span className={c.estado === 'recibida' ? 'badge-green' : 'badge-orange'} style={{ textTransform: 'capitalize' }}>{c.estado}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: '100%', maxWidth: 720, maxHeight: '92vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: '#1a1f36' }}>Nueva Compra</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', marginBottom: 4 }}>Proveedor</label>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#1a1f36' }}>Productos</span>
            ...
            <div style={{ background: '#f4f7fb', borderRadius: 8, padding: '12px 16px', marginBottom: 20 }}>
