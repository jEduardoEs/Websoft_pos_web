...
export default function CierresPage() {
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a1f36' }}>Cierre de Caja</h1>
        <p style={{ fontSize: 13, color: '#4a5568', marginTop: 3 }}>Registra el cierre diario de ventas</p>
      </div>
      ...
      <div className="card">
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #e2eaf4', fontWeight: 700, fontSize: 14 }}>Historial de cierres</div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Fecha', 'Período', 'Ventas', 'Efectivo', 'Tarjeta', 'Transferencia', 'Gran Total', 'Usuario'].map(h => (
                <th key={h} style={{ background: '#f4f7fb', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', padding: '9px 13px', textAlign: 'left', borderBottom: '1px solid #e2eaf4', whiteSpace: 'nowrap' }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              {cierres.length === 0 ? <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: '#9ca3af' }}>Sin cierres</td></tr>
              : cierres.map(c => (
                <tr key={c.id}>
                  <td style={{ padding: '10px 13px', fontSize: 12, borderBottom: '1px solid #f4f7fb', whiteSpace: 'nowrap' }}>{fmtDateTime(c.fecha)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 11, borderBottom: '1px solid #f4f7fb', color: '#4a5568', whiteSpace: 'nowrap' }}>{fmtDateTime(c.fechaInicio)} — {fmtDateTime(c.fechaFin)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f4f7fb' }}>{c.totalVentas}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f4f7fb' }}>{fmt(c.totalEfectivo)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f4f7fb' }}>{fmt(c.totalTarjeta)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f4f7fb' }}>{fmt(c.totalTransferencia)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 14, fontWeight: 700, borderBottom: '1px solid #f4f7fb', color: '#22c55e' }}>{fmt(c.granTotal)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 12, borderBottom: '1px solid #f4f7fb', color: '#4a5568' }}>{c.usuarioNombre}</td>
                </tr>
              ))}
            </tbody>
          </table>
