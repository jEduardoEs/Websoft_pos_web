...
export default function CierresPage() {
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#e2e8f0' }}>Cierre de Caja</h1>
        <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 3 }}>Registra el cierre diario de ventas</p>
      </div>
      ...
      <div className="card">
        <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(255,255,255,.06)', fontWeight: 700, fontSize: 14 }}>Historial de cierres</div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['Fecha', 'Período', 'Ventas', 'Efectivo', 'Tarjeta', 'Transferencia', 'Gran Total', 'Usuario'].map(h => (
                <th key={h} style={{ background: 'rgba(37,99,235,.1)', fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', padding: '9px 13px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,.06)', whiteSpace: 'nowrap' }}>{h}</th>
              ))}</tr>
            </thead>
            <tbody>
              ...
              : cierres.map(c => (
                <tr key={c.id}>
                  <td style={{ padding: '10px 13px', fontSize: 12, borderBottom: '1px solid rgba(255,255,255,.04)', whiteSpace: 'nowrap' }}>{fmtDateTime(c.fecha)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 11, borderBottom: '1px solid rgba(255,255,255,.04)', color: '#94a3b8', whiteSpace: 'nowrap' }}>{fmtDateTime(c.fechaInicio)} — {fmtDateTime(c.fechaFin)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid rgba(255,255,255,.04)' }}>{c.totalVentas}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid rgba(255,255,255,.04)' }}>{fmt(c.totalEfectivo)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid rgba(255,255,255,.04)' }}>{fmt(c.totalTarjeta)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid rgba(255,255,255,.04)' }}>{fmt(c.totalTransferencia)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 14, fontWeight: 700, borderBottom: '1px solid rgba(255,255,255,.04)', color: '#10b981' }}>{fmt(c.granTotal)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 12, borderBottom: '1px solid rgba(255,255,255,.04)', color: '#94a3b8' }}>{c.usuarioNombre}</td>
                </tr>
              ))}
            </tbody>
          </table>
