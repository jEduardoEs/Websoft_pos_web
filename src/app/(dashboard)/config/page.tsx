...
export default function ConfigPage() {
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1a1f36' }}>Configuración</h1>
        <p style={{ fontSize: 13, color: '#4a5568', marginTop: 3 }}>Personaliza tu sistema</p>
      </div>
        ...
        <div style={{ padding: 24 }}>
          {tab === 'empresa' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <p style={{ fontSize: 13, color: '#4a5568' }}>Información de tu empresa para los tickets y reportes.</p>
          ...
          {tab === 'ticket' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <p style={{ fontSize: 13, color: '#4a5568' }}>Configuración del ticket impreso.</p>
              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', marginBottom: 4 }}>Mensaje del ticket</label>
          ...
          {tab === 'sistema' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <p style={{ fontSize: 13, color: '#4a5568' }}>Parámetros del sistema.</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', marginBottom: 4 }}>IVA (%)</label>
                  <input className="input" type="number" value={cfg.iva_porcentaje || '12'} onChange={e => set('iva_porcentaje', e.target.value)} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', marginBottom: 4 }}>Símbolo moneda</label>
                  <input className="input" value={cfg.moneda_simbolo || 'Q'} onChange={e => set('moneda_simbolo', e.target.value)} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', marginBottom: 4 }}>Número siguiente factura</label>
                  <input className="input" type="number" value={cfg.numero_siguiente || '1'} onChange={e => set('numero_siguiente', e.target.value)} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#4a5568', textTransform: 'uppercase', marginBottom: 4 }}>Número siguiente compra</label>
