...
export default function ProyectosPage() {
  ...
  const handleEliminar = (id: number) => {
    ...
    if (esAdminOSupervisor) {
      ...
      setShowPinEliminar(id); setPinInput('')
    }
  }
                      ...
                      <td style={tdS} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', position: 'relative' }}>
                          ...
                          <button onClick={() => router.push(`/proyectos/${p.id}`)}
                            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', background: '#fff', border: '1.5px solid #d8d6cd', borderRight: 'none', borderRadius: '4px 0 0 4px', cursor: 'pointer', color: '#52524d', fontSize: 12, fontWeight: 600, fontFamily: 'inherit' }}>
                          ...
                          {/* Dropdown menu */}
                          {openMenuId === p.id && (
                            <div style={{ position: 'absolute', right: 0, top: '110%', background: '#fff', border: '1.5px solid #d8d6cd', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,.1)', zIndex: 50, minWidth: 150, overflow: 'hidden' }}>
                              <button onClick={() => { router.push(`/proyectos/${p.id}`); setOpenMenuId(null) }}
                                style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', padding: '9px 14px', fontSize: 12, fontWeight: 500, color: '#18181b', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}
                              ...
                              <button onClick={() => handleEliminar(p.id)}
                                style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', padding: '9px 14px', fontSize: 12, fontWeight: 500, color: '#b13a2e', background: 'none', border: 'none', borderTop: '1px solid #f1f5f9', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 10, padding: 28, width: '100%', maxWidth: 360, boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
            ...
            <p style={{ fontSize: 13, color: '#52524d', marginBottom: 20, lineHeight: 1.6 }}>
              Ingresa la contraseña de administrador para autorizar esta eliminación.
            </p>
