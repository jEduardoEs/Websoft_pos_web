  ...
  const abrir = async () => {
    ...
    try {
      const res = await fetch('/api/caja', {
        ...
        body: JSON.stringify({ accion: 'abrir', fondo: parseFloat(fondoInicial) || 0, notas: notasApertura }),
      })
      ...
      if (d.ok) {
        ...
        toast.error(d.error || 'Error al abrir caja')
      }
    } catch {
      setLoading(false)
  ...
  const movimiento = async (tipo: 'inyeccion' | 'retiro') => {
    ...
    try {
      const res = await fetch('/api/caja', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accion: tipo, monto: parseFloat(montoMov), motivo: motivoMov }),
      })
      const d = await res.json()
      setLoading(false)
      if (d.ok) {
        ...
        toast.error(d.error || 'Error al registrar movimiento')
      }
    } catch {
      setLoading(false)
  ...
  const cerrar = async () => {
    ...
    try {
      ...
      if (d.ok) {
        ...
        toast.error(d.error || 'Error al cerrar caja')
      }
    } catch {
      setLoading(false)
  ...
  const DifBadge = ({ dif, label }: { dif: number; label: string }) => (
    <div
      style={{
        padding: '6px 10px',
        borderRadius: 8,
        background: dif === 0 ? '#f0fdf4' : dif > 0 ? '#eff6ff' : '#fef2f2',
        border: `1px solid ${dif === 0 ? '#bbf7d0' : dif > 0 ? '#bfdbfe' : '#fecaca'}`,
        marginTop: 4,
      }}
    >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontSize: 12, color: '#64748b' }}>
                · desde {fmtDateTime(data.activa.fecha)} · {data.activa.usuarioNombre}
              </span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                {[
                  ...
                  { label: 'Total ventas', value: fmt(data.totalVentas), color: '#16a34a', sub: 'Todos los métodos' },
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div className="card" style={{ padding: 18 }}>
                  ...
                  >
                    <span>TOTAL</span>
                    <span style={{ color: '#2563eb' }}>{fmt(data.totalVentas)}</span>
                  </div>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 14 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div className="card" style={{ padding: 18, borderLeft: '4px solid #2563eb' }}>
                  <div style={{ fontWeight: 700, fontSize: 14, color: '#2563eb', marginBottom: 4 }}>
                    Inyección de capital
                  </div>
                  <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>
                    Agrega efectivo a la caja (cambio extra, fondo adicional)
                  </div>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <input
                      ...
                      placeholder="Ej: Fondo de cambio adicional"
                    />
                  </div>
                  <button className="btn-primary" style={{ width: '100%' }} onClick={() => movimiento('inyeccion')} disabled={loading}>
                    Registrar Inyección
                  </button>
                </div>
                ...
                <div className="card" style={{ padding: 18, borderLeft: '4px solid #d97706' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>
                    Saca efectivo para guardar en bodega o depositar al banco
                  </div>
                  <div style={{ marginBottom: 10 }}>
                    <label style={lbl}>Monto (Q)</label>
                    <input
                  ...
                  <button className="btn-warning" style={{ width: '100%' }} onClick={() => movimiento('retiro')} disabled={loading}>
                    Registrar Retiro
                  </button>
                </div>
              </div>
              ...
              <div className="card">
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr>
                        {['Hora', 'Tipo', 'Monto', 'Motivo', 'Usuario'].map(h => (
                          <th
                            key={h}
                            style={{
                              background: '#f8fafc',
                    ...
                    <tbody>
                      {data.movimientos.length === 0 ? (
                        ...
                        data.movimientos.map((m, i) => (
                          <tr key={i}>
                            <td style={{ padding: '9px 14px', fontSize: 12, borderBottom: '1px solid #f1f5f9', color: '#64748b', whiteSpace: 'nowrap' }}>
                              {fmtDateTime(m.fecha)}
                            </td>
                            <td style={{ padding: '9px 14px', borderBottom: '1px solid #f1f5f9' }}>
                              <span className={m.tipo === 'inyeccion' ? 'badge-blue' : 'badge-orange'}>
                                {m.tipo === 'inyeccion' ? 'Inyección' : 'Retiro'}
                              </span>
                            </td>
                            <td
                              style={{
                                ...
                                fontSize: 13,
                                borderBottom: '1px solid #f1f5f9',
                                color: m.tipo === 'inyeccion' ? '#2563eb' : '#d97706',
                              }}
                            >
                              {m.tipo === 'retiro' ? '-' : '+'}{fmt(m.monto)}
                            </td>
                            <td style={{ padding: '9px 14px', fontSize: 12, borderBottom: '1px solid #f1f5f9', color: '#475569' }}>
                              {m.motivo || '—'}
                            </td>
                            <td style={{ padding: '9px 14px', fontSize: 12, borderBottom: '1px solid #f1f5f9', color: '#64748b' }}>
                              {m.usuarioNombre}
                            </td>
                          </tr>
                        ))
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '420px 1fr', gap: 14 }}>
              <div className="card" style={{ padding: 24, borderTop: '3px solid #dc2626' }}>
                ...
                <div style={{ fontSize: 12, color: '#64748b', marginBottom: 18 }}>
                  Cuenta el dinero físicamente y registra el cierre del turno.
                </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={tarjetaBaucher}
                    onChange={e => setTarjetaBaucher(e.target.value)}
                    placeholder={`POS registro: ${fmt(data.ventasTarjeta)}`}
                  />
                  <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 3 }}>
                    POS registro: <strong>{fmt(data.ventasTarjeta)}</strong>
                  </div>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <input
                    ...
                    placeholder={`POS registro: ${fmt(data.ventasTransferencia)}`}
                  />
                  <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 3 }}>
                    POS registro: <strong>{fmt(data.ventasTransferencia)}</strong>
                  </div>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <textarea
                    ...
                    placeholder="Irregularidades, observaciones..."
                  />
                </div>
                      ...
                      {r.contado !== null && (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          >
                            {fmt(r.contado)}{' '}
                            {r.contado - r.sistema !== 0
                              ? `(${r.contado - r.sistema > 0 ? '+' : ''}${fmt(r.contado - r.sistema)})`
                              : ''}
                          </span>
                        </div>
                      )}
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 32, width: 440, boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
            ...
            >
              {cierreResult.diferencia === 0
                ? 'Caja cuadrada'
                : cierreResult.diferencia > 0
                ? `Sobrante: ${fmt(cierreResult.diferencia)}`
                : `Faltante: ${fmt(Math.abs(cierreResult.diferencia))}`}
            </div>
            <button className="btn-primary" style={{ width: '100%' }} onClick={() => setCierreResult(null)}>
              Listo
            </button>
          </div>
        </div>
