    ...
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#64748b', marginTop: 3 }}>Códigos de descuento para ventas</p>
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              : descuentos.map(d => (
                <tr key={d.id}>
                  <td style={{ padding: '10px 13px', fontSize: 13, fontWeight: 700, borderBottom: '1px solid #f1f5f9', color: '#2563eb', fontFamily: 'monospace' }}>{d.codigo}</td>
                  <td style={{ padding: '10px 13px', borderBottom: '1px solid #f1f5f9' }}><span className="badge-blue" style={{ textTransform: 'capitalize' }}>{d.tipo}</span></td>
                  <td style={{ padding: '10px 13px', fontSize: 13, fontWeight: 700, borderBottom: '1px solid #f1f5f9' }}>{d.tipo === 'porcentaje' ? `${d.valor}%` : fmt(d.valor)}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f1f5f9', color: '#64748b' }}>{d.minimoCompra > 0 ? fmt(d.minimoCompra) : '—'}</td>
                  <td style={{ padding: '10px 13px', fontSize: 13, borderBottom: '1px solid #f1f5f9' }}>{d.usosActuales}/{d.usosMaximos === 0 ? '∞' : d.usosMaximos}</td>
                  <td style={{ padding: '10px 13px', fontSize: 11, borderBottom: '1px solid #f1f5f9', color: '#64748b' }}>{d.fechaInicio ? `${fmtDate(d.fechaInicio)} — ${fmtDate(d.fechaFin)}` : 'Sin límite'}</td>
                  <td style={{ padding: '10px 13px', borderBottom: '1px solid #f1f5f9' }}><span className={d.activo ? 'badge-green' : 'badge-red'}>{d.activo ? 'Activo' : 'Inactivo'}</span></td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#ffffff', borderRadius: 14, padding: 28, width: '100%', maxWidth: 500 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: '#0f172a' }}>Nuevo Código de Descuento</h3>
