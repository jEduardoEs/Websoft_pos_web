...
export default function InventarioPage() {
  ...
  const [margin, setMargin] = useState<number>(0.30)
  const [ivaAmount, setIvaAmount] = useState<string>('')

  // Automatic calculation of Price and IVA from Cost + Margin
  useEffect(() => {
    const costNum = parseFloat(form.costo)
    if (!isNaN(costNum) && costNum > 0 && typeof margin === 'number') {
      const base = costNum * (1 + margin)
      const ivaAmt = base * 0.05
      const totalPrice = (base + ivaAmt).toFixed(2)
      setIvaAmount(ivaAmt.toFixed(2))
      setForm(prev => {
        if (prev.precio === totalPrice) return prev
        return { ...prev, precio: totalPrice }
      })
    }
  }, [form.costo, margin])
  ...
  const openNew = () => { setForm(empty); setMargin(0.30); setShowModal(true) }
  const openEdit = (p: Produto) => {
    ...
    if (p.costo > 0 && p.precio > 0) {
      const calcMargin = (p.precio / 1.05 - p.costo) / p.costo
      if (calcMargin > 0) setMargin(Number(calcMargin.toFixed(2)))
    }
    setShowModal(true)
  }
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: 20, overflowY: 'auto' }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: 28, width: '100%', maxWidth: 600, margin: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              {/* IVA y Precio Venta Auto-calculado */}
              <div>
                <label style={lbl}>IVA (5% Incluido)</label>
                <input className="input" type="number" value={ivaAmount} readOnly disabled placeholder="0.00" style={{ background: '#f8fafc', color: '#64748b' }} />
              </div>
              <div>
                <label style={lbl}>Precio Venta (Con IVA)</label>
                <input className="input" type="number" step="0.01" value={form.precio} onChange={e => setF('precio', e.target.value)} placeholder="0.00" style={{ fontWeight: 700, color: '#1581E3' }} />
              </div>
