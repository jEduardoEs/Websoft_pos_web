...
// API key: Config (clave: dtevia_api_key) con fallback a env DTEVIA_API_KEY
// qapi_test_... = sandbox (certificador MOCK) | qapi_live_... = producción SAT
...
function buildInvoice(input: FELInput) {
  ...
  // DTEvia calcula el IVA automáticamente — enviamos precio con IVA incluido
  const items = input.items.map(it => {
    return {
      ...
      precio_unitario: Number(it.precioUnitario.toFixed(6)),
      descuento: Number((it.descuento || 0).toFixed(6)),
    }
  })
...
export async function emitirFEL(input: FELInput): Promise<FELResponse> {
  ...
  if (!apiKey) {
    console.error('[FEL] API key DTEvia no configurada (Config dtevia_api_key o env DTEVIA_API_KEY)')
