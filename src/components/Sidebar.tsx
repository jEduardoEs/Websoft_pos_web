...
const NAV = [
  ...
  { href: '/compras', label: 'Compras', icon: 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17', roles: ['admin'] },
...
export default function Sidebar({ role }: { role: string }) {
  ...
  return (
    <aside style={{
      width: 200, background: '#fff', borderRight: '1px solid #e2eaf4',
      display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0
    }}>
      <nav style={{ flex: 1, overflowY: 'auto', padding: '8px 6px' }}>
        {NAV.filter(n => n.roles.includes(role)).map(({ href, label, icon }) => {
          ...
          return (
            <Link key={href} href={href} style={{ textDecoration: 'none' }}>
              <div style={{
                ...
                color: active ? '#2B7FD4' : '#4a5568',
                background: active ? 'rgba(43,127,212,.1)' : 'transparent',
                transition: 'all .15s',
              }}>
