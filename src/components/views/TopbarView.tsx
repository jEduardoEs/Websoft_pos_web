...
export default function TopbarView({ user, onSignOut }: TopbarViewProps) {
  return (
    <div style={{
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
        {user.role === 'admin' && (
          <div className="topbar-inactivity" style={{ fontSize: 10, color: '#8a887e', background: '#f4f3ef', border: '1px solid #e3c3bd', borderRadius: 4, padding: '3px 8px' }}>
            Sesion expira 30min
          </div>
        )}
        ...
        <button onClick={onSignOut} style={{ background: '#fff', color: '#b13a2e', border: '1.5px solid #e3c3bd', padding: '5px 12px', borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>
          Salir
        </button>
      </div>
